...
public class AppTest {
    ...
    public void timeoutTest() throws InterruptedException {
        ...
        //chamo a empty queue porque este passa no teste se as threads sairem por timeout, returnarem false
        emptyQueue(tq);
    }
    ...
    // teste para testar que todos os objectos que são colocados na fila, são retirados e não falta nenhum
    // testando também as instâncias da mesma
    @Test
    ...
    public void emptyQueue(TransferQueue<Contentor> tq) throws InterruptedException {
        Contentor msg = new Contentor(-1);
        ...
        Thread a = new Thread(() -> {
            try {
                if(tq.transfer(msg, 1000))
                    timeout[0] = false;
                // mete o timeout[0] a true se o tq.transfer retornou false, logo a queue Take está vazia
                // podemos supor isso porque este saiu por timeout (return false)
                else timeout[0] = true;
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        ...
        Thread b = new Thread(() -> {
            try {
                if(tq.take(1000, answer))
                    timeout[1] = false;
                // mete o timeout[0] a true se o tq.take retornou false, logo a queue Transfer está vazia
                // podemos supor isso porque este saiu por timeout (return false)
                else timeout[1] = true;
            } catch (InterruptedException e) {
                e.printStackTrace();
