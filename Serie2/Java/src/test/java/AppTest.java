...
public class AppTest {
    @Test
    public void testMichaelScottQueue() throws InterruptedException {
    ...
    @Test
    public void testLockFreeDualQueue() throws InterruptedException {
        final int CONSUMER_THREADS = 2;
        final int PRODUCER_THREADS = 1;
    ...
    public void testSafeRefCountedHolder() throws InterruptedException {
        IDisposable id = ()->{};
        SafeRefCountedHolder<IDisposable> sr = new SafeRefCountedHolder(id);
        int numberOfThread = 100000;
        Thread [] thread = new Thread[numberOfThread];
        IDisposable [] results = new IDisposable[numberOfThread];
        for(int i = 0; i < numberOfThread; i++){
            final int local = i;
            thread[i] = new Thread(()->{
                    sr.addRef();
                    results[local] = sr.getValue();
                    sr.ReleaseRef();
            });
            thread[i].start();
        }
